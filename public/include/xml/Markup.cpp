...
bool CMarkupSTL::WriteTextFile( const char* szFileName, const string& strDoc, string* pstrError, int* pnFlags )
{
...
int CMarkupSTL::x_ParseNode( CMarkupSTL::TokenPos& token, CMarkupSTL::NodePos& node )
{
	...
	register unsigned int cD = (unsigned)(int)*pDoc;		//2012-08-31  LiYingtao  PC-LINT
	if ( ! cD )
	{
		node.nLength = 0;
		node.nNodeType = 0;
	...
	while ( 1 )
	{
		cD = (unsigned)(int)*pDoc;		//2012-08-31  LiYingtao  PC-LINT
		if ( ! cD )
		{
...
bool CMarkupSTL::x_CreateNode( string& strNode, int nNodeType, const char* szText )
{
	...
	case MNT_CDATA_SECTION:
		...
		break;
	default:
		break;		//2012-08-31  LiYingtao  PC-LINT
	}
	return true;
}
...
string CMarkupSTL::x_GetSubDoc( int iPos ) const
{
	if ( iPos )
	{
		...
		if ( ! x_FindAny(szDoc,nChar) || szDoc[nChar] == '<' )
			nNext = nChar;
		return m_strDoc.substr( nStart, unsigned(nNext - nStart) );
	}
	return "";
}
...
bool CMarkupSTL::x_AddSubDoc( const char* szSubDoc, int nFlags )
{
	...
	if ( bWellFormed )
	{
		...
		node.strMeta.assign( &szSubDoc[nExtractStart], (unsigned int)nExtractLength );
	}
	else
	{
...
void CMarkupSTL::x_LinkElem( int iPosParent, int iPosBefore, int iPos ) const
{
...
int CMarkupSTL::x_ReleaseSubDoc( int iPos )
{
	...
	//return iPosNext;		//2012-08-31  LiYingtao  PC-LINT
}
